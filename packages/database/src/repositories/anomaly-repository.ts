...
export async function createAnomaly(
	...
	try {
		...
		propagateError(err);
		throw AppError.database(`Failed to create anomaly: ${(err as Error).message}`, {
			cause: err,
		});
	}
}
...
export async function createAnomalies(
	...
	try {
		...
		propagateError(err);
		throw AppError.database(`Failed to create anomalies: ${(err as Error).message}`, {
			cause: err,
		});
	}
}
...
export async function deleteExpiredAnomalies(dbClient: DatabaseInstance) {
	try {
		...
		const result = await dbClient
			...
			.where(
				and(
					eq(anomaly.isAcknowledged, true),
					lt(anomaly.expiresAt, now),
				),
			)
