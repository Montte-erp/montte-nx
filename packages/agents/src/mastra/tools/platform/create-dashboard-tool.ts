...
export const createDashboardTool = createTool({
   ...
   inputSchema: z.object({
      name: z
         ...
         .min(1)
         .max(100)
         .describe("Name for the new dashboard (1-100 characters)"),
   }),
   ...
   execute: async (inputData, context) => {
      const db = context?.requestContext?.get("db") as
         | DatabaseInstance
         | undefined;
      const teamId = context?.requestContext?.get("teamId") as
         | string
         | undefined;
      const organizationId = context?.requestContext?.get("organizationId") as
         | string
         | undefined;
      ...
      if (!db || !teamId || !organizationId || !userId) {
         return {
            ...
            name: inputData.name,
            message:
               "Missing required context: db, teamId, organizationId, or userId not available.",
         };
      }
