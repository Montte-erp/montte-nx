...
import { bankAccount } from "@packages/database/schema";
import { category, transactionCategory } from "@packages/database/schema";
...
const quickActionsConfigSchema = z.object({
	...
	actions: z.array(z.enum(["new_transaction", "reports", "payables", "receivables"])),
});
...
export const dashboardRouter = router({
	...
	update: protectedProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			return updateDashboard(resolvedCtx.db, input.id, {
				defaultFilters: input.defaultFilters as DashboardFilterConfig | undefined,
	...
	updateSavedInsight: protectedProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			if (!insight || insight.organizationId !== resolvedCtx.organizationId) {
	...
	getAllSavedInsights: protectedProcedure
		...
		.query(async ({ ctx, input }) => {
			...
			return findSavedInsightsByOrganizationId(resolvedCtx.db, organizationId, {
				search: input?.search,
			});
		}),
...
async function queryTransactionInsight(
	...
	if (config.breakdown) {
		if (config.breakdown.field === "categoryId") {
			...
			result.breakdown = breakdownResult.map((row: { label: string; value: number; color: string | null }) => ({
			...
			result.breakdown = breakdownResult.map((row: { label: string | null; value: number }) => ({
			...
			result.breakdown = breakdownResult.map((row: { label: string; value: number }) => ({
				color: row.label === "income" ? "#10b981" : row.label === "expense" ? "#ef4444" : "#8884d8",
				label: row.label,
				value: row.value,
			}));
		}
		...
		with: {
			periods: {
				where: (p: any, { and: andOp, gte: gteOp, lte: lteOp }: any) =>
					andOp(gteOp(p.periodStart, startDate), lteOp(p.periodEnd, endDate)),
			},
		},
	});
...
async function queryBankAccountInsight(
	...
	if (globalFilters?.bankAccountIds?.length) {
		baseConditions.push(inArray(bankAccount.id, globalFilters.bankAccountIds));
	}
	...
	const accountBalances = await Promise.all(
		accounts.map(async (account: { id: string; name: string | null; bank: string | null }) => {
