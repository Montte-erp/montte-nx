...
export async function createFinancialGoal(teamId: string, data: CreateFinancialGoalInput) {
...
export async function getFinancialGoal(id: string, teamId: string) {
  try {
    const [goal] = await db
      .select()
      .from(financialGoals)
      .where(and(eq(financialGoals.id, id), eq(financialGoals.teamId, teamId)));
    return goal ?? null;
  } catch (err) {
    propagateError(err);
...
export async function listFinancialGoals(teamId: string, opts?: { isCompleted?: boolean }) {
  try {
...
export async function createGoalMovement(goalId: string, data: CreateGoalMovementInput) {
  const validated = validateInput(createGoalMovementSchema, data);

  return await db.transaction(async (tx) => {
    const [goal] = await tx.select().from(financialGoals).where(eq(financialGoals.id, goalId));
    ...
    const delta =
      validated.type === "deposit" ? Number(validated.amount) : -Number(validated.amount);
...
export async function deleteGoalMovement(id: string) {
  return await db.transaction(async (tx) => {
    ...
    await tx.delete(financialGoalMovements).where(eq(financialGoalMovements.id, id));
