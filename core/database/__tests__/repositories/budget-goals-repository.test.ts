...
function validCreateInput(categoryId: string, overrides: Record<string, unknown> = {}) {
  return {
    categoryId,
    month: 3,
    year: 2026,
...
describe("budget-goals-repository", () => {
  describe("createBudgetGoal", () => {
    it("creates with correct fields", async () => {
      ...
      const goal = await repo.createBudgetGoal(teamId, validCreateInput(cat.id));
    ...
    it("rejects income category", async () => {
      ...
      await expect(repo.createBudgetGoal(teamId, validCreateInput(cat.id))).rejects.toThrow(
        /despesa/,
      );
    });
    ...
    it("rejects non-existent category", async () => {
      ...
      await expect(
        repo.createBudgetGoal(teamId, validCreateInput(crypto.randomUUID())),
    ...
    it("rejects invalid month", async () => {
      ...
      await expect(
        repo.createBudgetGoal(teamId, validCreateInput(cat.id, { month: 13 })),
      ).rejects.toThrow();
    });
  ...
  describe("getBudgetGoal", () => {
    it("gets by id and teamId", async () => {
      const teamId = randomTeamId();
      const cat = await createExpenseCategory(teamId);
      const created = await repo.createBudgetGoal(teamId, validCreateInput(cat.id));
    ...
    it("returns null for non-existent", async () => {
      const found = await repo.getBudgetGoal(crypto.randomUUID(), randomTeamId());
  ...
  describe("listBudgetGoals", () => {
    ...
    it("includes subcategory spending", async () => {
      const teamId = randomTeamId();
      const parent = await createExpenseCategory(teamId, "Alimentação Parent");
      ...
      await repo.createBudgetGoal(teamId, validCreateInput(parent.id, { limitAmount: "500.00" }));
  ...
  describe("copyPreviousMonth", () => {
    it("copies from one month to another", async () => {
      ...
      await repo.createBudgetGoal(teamId, validCreateInput(cat.id, { month: 2, year: 2026 }));
    ...
    it("skips duplicates", async () => {
      ...
      await repo.createBudgetGoal(teamId, validCreateInput(cat.id, { month: 2, year: 2026 }));
      await repo.createBudgetGoal(teamId, validCreateInput(cat.id, { month: 3, year: 2026 }));
