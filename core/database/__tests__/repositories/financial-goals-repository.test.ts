...
describe("financial-goals-repository", () => {
  describe("createFinancialGoal", () => {
    ...
    it("rejects name shorter than 2 chars", async () => {
      const teamId = randomTeamId();
      await expect(repo.createFinancialGoal(teamId, validGoalInput({ name: "A" }))).rejects.toThrow(
        /validation failed/,
      );
    });
  ...
  describe("getFinancialGoal", () => {
    it("gets by id and teamId", async () => {
      const teamId = randomTeamId();
      const created = await repo.createFinancialGoal(teamId, validGoalInput());
    ...
    it("returns null for non-existent", async () => {
      const found = await repo.getFinancialGoal(crypto.randomUUID(), randomTeamId());
  ...
  describe("listFinancialGoals", () => {
    it("lists all for team", async () => {
      const teamId = randomTeamId();
      await repo.createFinancialGoal(teamId, validGoalInput({ name: "Goal A" }));
      await repo.createFinancialGoal(teamId, validGoalInput({ name: "Goal B" }));

      const list = await repo.listFinancialGoals(teamId);
    ...
    it("filters by isCompleted", async () => {
      ...
      await repo.createGoalMovement(completeGoal.id, {
        type: "deposit",
        amount: "100.00",
        date: "2026-01-15",
      });
      ...
      const incomplete = await repo.listFinancialGoals(teamId, {
        isCompleted: false,
      });
  ...
  describe("createGoalMovement", () => {
    it("deposit increases currentAmount", async () => {
      ...
      const { goal: updated } = await repo.createGoalMovement(goal.id, {
        type: "deposit",
        amount: "250.00",
        date: "2026-01-15",
      });
  ...
  describe("deleteGoalMovement", () => {
    it("reverts deposit delta", async () => {
      ...
      const after = await repo.getFinancialGoal(goal.id, teamId);
