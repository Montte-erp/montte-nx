...
export function SankeyChart({ data, height = 400, className }: SankeyChartProps) {
	const { nodes, positions, paths } = useMemo(() => {
		...
		for (const node of sourceNodes) {
			const nodeHeight = Math.max(20, (node.value / totalSourceValue) * availableHeight);
		...
		for (const node of targetNodes) {
			const nodeHeight = Math.max(20, (node.value / totalTargetValue) * availableHeight);
			positions.set(node.id, {
		...
		const paths = data.links.map((link, index) => {
			...
			return {
				...
				path,
				value: link.value,
				color: sourceNode.color ?? DEFAULT_COLORS[index % DEFAULT_COLORS.length],
				...
				<g className="links">
					{paths.map((link) => (
						<g key={link?.id}>
							...
							/>
							<title>
								{link?.sourceName} → {link?.targetName}: {formatDecimalCurrency(link?.value ?? 0)}
							</title>
						</g>
					))}
				</g>
				...
				<g className="source-nodes">
					{data.nodes
						...
						.map((node, index) => {
							...
							if (!pos) return null;
							const color = node.color ?? DEFAULT_COLORS[index % DEFAULT_COLORS.length];
				...
				<g className="target-nodes">
					{data.nodes
						...
						.map((node, index) => {
							const pos = positions.get(node.id);
							if (!pos) return null;
							const color = node.color ?? DEFAULT_COLORS[(index + 3) % DEFAULT_COLORS.length];
