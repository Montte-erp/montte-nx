...
export function HeatmapChart({
	...
	return (
		<TooltipProvider delayDuration={100}>
			<div className={cn("w-full overflow-x-auto", className)} style={{ minHeight: height }}>
				<div className="min-w-[400px]">
					...
					<div className="flex flex-col gap-1">
						{hourSlots.map((hours, slotIndex) => (
							<div key={`slot-${slotIndex + 1}`} className="flex items-center gap-1">
								{/* Hour label */}
											...
											<TooltipTrigger asChild>
												<div
													className={cn(
														...
														getIntensityClass(totalValue, data.maxValue, colorScale),
													)}
												/>
											</TooltipTrigger>
											<TooltipContent>
												<div className="text-xs">
													<p className="font-medium">
														{xLabels[day]} {HOUR_LABELS[slotIndex]} - {(hours[2] ?? 0) + 1}h
													</p>
													{totalCount > 0 ? (
														<>
															<p>{formatDecimalCurrency(totalValue)}</p>
															<p className="text-muted-foreground">
																{totalCount} transacao{totalCount > 1 ? "es" : ""}
															</p>
														</>
													) : (
														<p className="text-muted-foreground">Sem transacoes</p>
													)}
												</div>
											</TooltipContent>
										</Tooltip>
						...
						<div className="flex gap-1">
							{["empty", "low", "medium", "high", "max"].map((level) => (
								<div
									...
									className={cn(
										"w-4 h-4 rounded-sm",
										COLOR_SCALES[colorScale][level as keyof typeof COLOR_SCALES.green],
									)}
								/>
							))}
						</div>
