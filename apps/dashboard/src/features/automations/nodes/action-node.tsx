...
import type {
   ...
   AutomationEdge,
   AutomationNode,
} from "../hooks/use-flow-serialization";
import {
   validateActionNode,
   validateDataDependencies,
   validateEmailAttachmentDependency,
} from "../hooks/use-node-validation";
...
export function ActionNode({ id, data }: NodeProps<ActionNodeType>) {
   ...
   const label = actionDef?.label ?? data.label;
               ...
               <div className="flex items-center gap-1.5 mt-1">
                     ...
                     <Tooltip>
                        ...
                        <TooltipContent className="text-xs" side="bottom">
                           {dataFlow?.requires ? (
                              <span>
                                 Requer:{" "}
                                 {dataFlow.requiresLabel ?? dataFlow.requires}
                              </span>
                           ) : (
                              <span>
                                 Entrada opcional:{" "}
                                 {dataFlow?.optionalInputsLabel}
                              </span>
                           )}
                        </TooltipContent>
                     </Tooltip>
                  ...
                  {hasOutput && (
                     <Tooltip>
                        ...
                        <TooltipContent className="text-xs" side="bottom">
                           Produz:{" "}
                           {dataFlow?.producesLabel ?? dataFlow?.produces}
                        </TooltipContent>
                     </Tooltip>
                  )}
               </div>
