...
const bankAccountSchema = createInsertSchema(bankAccounts).pick({
	name: true,
	type: true,
	color: true,
	iconUrl: true,
...
export const getById = protectedProcedure
	...
	.handler(async ({ context, input }) => {
		...
		if (!account || account.teamId !== teamId) {
			throw new ORPCError("NOT_FOUND", { message: "Conta bancária não encontrada." });
		}
		const currentBalance = await computeBankAccountBalance(db, account.id, account.initialBalance);
		return { ...account, currentBalance };
	});

export const update = protectedProcedure
	.input(z.object({ id: z.string().uuid() }).merge(bankAccountSchema.partial()))
	.handler(async ({ context, input }) => {
		const { db, teamId } = context;
		const account = await getBankAccount(db, input.id);
		if (!account || account.teamId !== teamId) {
