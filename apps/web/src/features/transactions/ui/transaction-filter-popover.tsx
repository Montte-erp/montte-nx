...
function conditionToRow(c: Condition): FilterRow {
	const rawValue =
		"value" in c && c.value !== undefined && c.value !== null
			...
			: "";
	const weight =
		c.options?.weight !== undefined
			? Math.round(c.options.weight * 100)
			: 50;
	return {
		id: c.id,
		field: c.field,
...
function rowToCondition(row: FilterRow, advanced: boolean): Condition {
	...
	if (def.type === "number") {
		return {
			...
			value:
				noValue || row.value === "" ? undefined : Number(row.value),
			options,
		};
	}
	return {
	...
	const handleOpenChange = (next: boolean) => {
		if (next && value) {
			setRows(
				...
				) as Condition[]).map(conditionToRow),
			);
					...
					<div className="flex items-center gap-3">
						...
						<div className="flex items-center gap-1.5 text-xs text-muted-foreground">
							...
							<ToggleGroup
								...
								variant="outline"
							>
								<ToggleGroupItem className="h-6 px-2 text-xs" value="AND">
									E
								</ToggleGroupItem>
								<ToggleGroupItem className="h-6 px-2 text-xs" value="OR">
									OU
								</ToggleGroupItem>
							</ToggleGroup>
						</div>
						...
						rows.map((row) => {
							...
							return (
								...
								>
									{/* Property select */}
									<Select
										onValueChange={(v) => handleFieldChange(row.id, v)}
										value={row.field}
									>
										...
										<SelectContent>
											{FILTER_PROPERTIES.map((prop) => (
												<SelectItem key={prop.field} value={prop.field}>
													{prop.label}
												</SelectItem>
											))}
										</SelectContent>
									...
									{/* Operator select */}
									<Select
										onValueChange={(v) => handleOperatorChange(row.id, v)}
										value={row.operator}
									>
										<SelectTrigger className="h-7 text-xs">
											<SelectValue />
									...
									{advanced && (
										<div className="flex items-center gap-1">
											<Slider
												...
												min={0}
												onValueChange={([v]) => {
													if (v !== undefined) handleWeightChange(row.id, v);
												}}
												step={5}
												value={[row.weight]}
											/>
