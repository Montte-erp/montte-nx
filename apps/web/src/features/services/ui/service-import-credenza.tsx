...
function ConfirmStep({ methods, rows, onClose }: ConfirmStepProps) {
   ...
   function handleImport() {
      ...
      const items = validRows.map((row) => ({
         ...
         basePrice: row.priceCents != null ? String(row.priceCents / 100) : "0",
      }));
      ...
      bulkCreateMutation.mutate(
         { items },
         {
            onSuccess: (inserted) => {
               toast.success(
                  `${inserted.length} serviço(s) importado(s) com sucesso.`,
               );
               onClose?.();
            },
            onError: (e) =>
               toast.error(e.message || "Erro ao importar serviços."),
         },
      );
   }
