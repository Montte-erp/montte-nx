...
vi.mock("@core/database/repositories/property-definition-repository");
...
describe("create", () => {
   ...
   it("creates property definition successfully", async () => {
      ...
      vi.mocked(createPropertyDefinition).mockResolvedValueOnce(
         propertyDefinition,
      );

      const ctx = createTestContext();
      const result = await call(propertyDefinitionsRouter.create, input, {
         context: ctx,
      });
...
describe("list", () => {
   it("returns property definitions list", async () => {
      ...
      const ctx = createTestContext();
      const result = await call(propertyDefinitionsRouter.list, undefined, {
         context: ctx,
      });
...
describe("update", () => {
   ...
   it("updates property definition successfully", async () => {
      ...
      const ctx = createTestContext();
      const result = await call(propertyDefinitionsRouter.update, input, {
         context: ctx,
      });
