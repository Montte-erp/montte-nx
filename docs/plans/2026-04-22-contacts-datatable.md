# Contacts DataTable Migration

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Replace `DataTable` + `SelectionActionBar` + manual type filter buttons with `DataTableRoot` + `DataTableBulkActions` + `DataTableExternalFilter`. The inline type filter buttons in `ContactsPage` become `DataTableExternalFilter` inside `DataTableRoot`.

**Architecture:** `DataTableRoot` wraps the table. Type filter buttons (Todos, Clientes, Fornecedores, Ambos) become `DataTableExternalFilter` components registered in the store and surfaced via `DataTableToolbar`'s filter dropdown. Bulk delete moves to `DataTableBulkActions`.

**Tech Stack:** TanStack Router, TanStack Query, `DataTableRoot` + `DataTableContent` + `DataTableEmptyState` + `DataTableBulkActions` + `DataTableExternalFilter` + `DataTableSkeleton` + `DataTableToolbar` from `@/components/data-table/`

---

### Task 1: Rewrite `contacts.tsx` to use `DataTableRoot`
...
Key changes:
- Removed `createLocalStorageState`, `DataTableStoredState`, `SelectionActionBar`, `SelectionActionButton`, `useRowSelection` from `@packages/ui`
- Removed the inline type filter buttons `div` from `ContactsPage` — now `DataTableExternalFilter` inside `DataTableRoot`
- `ContactsPage` no longer needs `navigate` — `ContactsList` calls `Route.useNavigate()` directly
- `BulkDelete` now uses `DataTableBulkActions` + `openAlertDialog` for confirmation
- `ContactsSkeleton` uses `DataTableSkeleton`

**Step 2: TypeCheck**

```bash
bun run typecheck 2>&1 | grep -A3 "contacts"
```
...
git commit -m "feat(contacts): migrate to DataTableRoot composable pattern"
```
