# Credit Cards DataTable Migration

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Replace the old monolithic `DataTable` + `SelectionActionBar` + `useRowSelection` from `@packages/ui` with `DataTableRoot` + `DataTableBulkActions` + `DataTablePagination` + `DataTableSkeleton`. Keep `renderExpandedRow` for `CreditCardFaturaRow`.

**Architecture:** `DataTableRoot` manages all internal state. `DataTableBulkActions` replaces `SelectionActionBar` and `useRowSelection`. `DataTablePagination` replaces the `pagination` prop. The `renderExpandedRow` pattern is passed directly to `DataTableContent`.

**Tech Stack:** TanStack Router, TanStack Query, `DataTableRoot` + `DataTableContent` + `DataTableEmptyState` + `DataTableBulkActions` + `DataTablePagination` + `DataTableSkeleton` + `DataTableToolbar` from `@/components/data-table/`

---

### Task 1: Rewrite `credit-cards.tsx` to use `DataTableRoot`
...
Path: `apps/web/src/routes/_authenticated/$slug/$teamSlug/_dashboard/credit-cards.tsx`

Current issues to fix:
- Uses `DataTable` + `DataTableStoredState` from `@packages/ui/components/data-table`
- Uses `SelectionActionBar` + `SelectionActionButton` from `@packages/ui/components/selection-action-bar`
- Uses `useRowSelection` from `@packages/ui/hooks/use-row-selection`
- Uses `createLocalStorageState` for manual table state
- Manual `OnChangeFn<ColumnFiltersState>` handler
...
Note: `CreditCardFormSkeleton` uses `Skeleton` from `@packages/ui/components/skeleton` — add that import.

**Step 3: Verify TypeScript**

```bash
bun run typecheck 2>&1 | grep -A3 "credit-cards"
```
...
git commit -m "feat(credit-cards): migrate to DataTableRoot composable pattern"
```
