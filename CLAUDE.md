...
**Rules:**
- Schema at **module level**, never inside component — recreating on render breaks validators.
- `isInvalid` must check `errors.length > 0`, not `!isValid` — avoids false positives.
- Always pass `id={field.name}`, `name={field.name}`, and `aria-invalid={isInvalid}` to every input.
- Always pass `htmlFor={field.name}` to every `<FieldLabel>` — required for label-input association.
- Use `children={(field) => ...}` prop syntax on `form.Field`, not JSX children.
- Use `form.Subscribe selector={(s) => [s.canSubmit, s.isSubmitting]}` for submit button disabled state.
    ...
    <form onSubmit={(e) => { e.preventDefault(); e.stopPropagation(); form.handleSubmit(); }}>
      ...
      <form.Subscribe selector={(s) => [s.canSubmit, s.isSubmitting]}>
        {([canSubmit, isSubmitting]) => (
          <Button type="submit" disabled={!canSubmit || isSubmitting}>Salvar</Button>
        )}
      </form.Subscribe>
    </form>
  );
