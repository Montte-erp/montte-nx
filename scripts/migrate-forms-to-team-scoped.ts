/**
 * Migrate Forms to Team-Scoped
 *
 * Backfills teamId for all existing forms and form_submissions by assigning them
 * to the organization's first team (or creating a "Default" team if none exists).
 *
 * Usage:
 *   bun run scripts/migrate-forms-to-team-scoped.ts run --dry-run
 *   bun run scripts/migrate-forms-to-team-scoped.ts run
 *   bun run scripts/migrate-forms-to-team-scoped.ts run --env production
 *   bun run scripts/migrate-forms-to-team-scoped.ts check
 */
...
import { forms, formSubmissions } from "@core/database/schemas/forms";
...
async function runMigration(env: string, dryRun: boolean) {
   console.log(colors.blue("--- Migrate Forms to Team-Scoped ---\n"));
   ...
   for (const org of orgs) {
      ...
      // Count forms without teamId
      const formsToUpdate = await db
         .select()
         .from(forms)
         .where(and(eq(forms.organizationId, org.id), isNull(forms.teamId)));

      if (formsToUpdate.length > 0) {
         console.log(
            colors.cyan(
               `   ${org.name}: ${formsToUpdate.length} forms → team "${defaultTeam.name}"`,
            ),
         );

         if (!dryRun) {
            // Update only forms that have null teamId
            await db
               .update(forms)
               .set({ teamId: defaultTeam.id })
               .where(
                  and(eq(forms.organizationId, org.id), isNull(forms.teamId)),
               );
         }

         totalFormsUpdated += formsToUpdate.length;
      }

      // Count form submissions without teamId
      const submissionsToUpdate = await db
         .select()
         .from(formSubmissions)
         .where(
            and(
               eq(formSubmissions.organizationId, org.id),
               isNull(formSubmissions.teamId),
            ),
         );
      ...
      if (submissionsToUpdate.length > 0) {
         console.log(
            colors.cyan(
               `   ${org.name}: ${submissionsToUpdate.length} form submissions → team "${defaultTeam.name}"`,
            ),
         );

         if (!dryRun) {
            // Update only submissions that have null teamId
            await db
         ...
         totalSubmissionsUpdated += submissionsToUpdate.length;
      }
   }
   ...
   console.log(colors.green(`   ✓ Forms updated: ${totalFormsUpdated}`));
   console.log(
      colors.green(`   ✓ Form submissions updated: ${totalSubmissionsUpdated}`),
   );
...
program
   .name("migrate-forms-to-team-scoped")
   .description("Backfill teamId for all existing forms and form submissions")
   .version("1.0.0");
