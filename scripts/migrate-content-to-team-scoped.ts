/**
 * Migrate Content to Team-Scoped
 *
 * Backfills teamId for all existing content by assigning them to the
 * organization's first team (or creating a "Default" team if none exists).
 *
 * Usage:
 *   bun run scripts/migrate-content-to-team-scoped.ts run --dry-run
 *   bun run scripts/migrate-content-to-team-scoped.ts run
 *   bun run scripts/migrate-content-to-team-scoped.ts run --env production
 *   bun run scripts/migrate-content-to-team-scoped.ts check
 */
...
async function runMigration(env: string, dryRun: boolean) {
   console.log(colors.blue("--- Migrate Content to Team-Scoped ---\n"));
   ...
   for (const org of orgs) {
      ...
      // Count content without teamId
      const contentToUpdate = await db
         .select()
         .from(content)
         .where(
            and(eq(content.organizationId, org.id), isNull(content.teamId)),
         );

      if (contentToUpdate.length > 0) {
         console.log(
            colors.cyan(
               `   ${org.name}: ${contentToUpdate.length} content items → team "${defaultTeam.name}"`,
            ),
         );

         if (!dryRun) {
            // Update only content that has null teamId
            await db
               .update(content)
               ...
               .where(
                  and(
                     ...
                     isNull(content.teamId),
                  ),
               );
         }

         totalUpdated += contentToUpdate.length;
      }
   }
   ...
   console.log(colors.green(`   ✓ Content items updated: ${totalUpdated}`));
...
program
   .name("migrate-content-to-team-scoped")
   .description("Backfill teamId for all existing content")
   .version("1.0.0");
